...
# review_ids_to_keep = set([
#     191688, 191689, 191693, 191694, 191695, 191696,
#     191697, 191701, 191705, 191706,
#     191707, 191710, 191711, 191714, 191716,
#     191721, 191722, 191725, 191726, 191728,
#     191729, 191731, 191734, 191736, 191737,
#     191738, 191739, 191741, 191747, 191748,
#     191750, 191751, 191760, 191762, 191763,
#     191766, 191768, 191770, 191771, 191772,
#     191773, 191775, 191777, 191781, 191787,
#     191788, 191797
# ])

review_ids_to_keep = set([6424995])


def filter_json_by_review_ids(filepath, ids_to_keep):
    matching = []
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            try:
                obj = json.loads(line)
                if obj.get("review_id") in ids_to_keep:
                    matching.append(obj)
...
#checkpoint_file = "/Users/lorenaraichle/Developer/ABSA/PyABSA/data/checkpoint_file_FT.txt"
...
def fetch_existing_review_id(review_id):
    ...
    cur = conn.cursor()
    query = "SELECT aspect FROM aspect_sentiment_results WHERE review_id = %s;"
...
def process_and_save_chunk(chunk_idx, df_chunk):
    ...
    print(f"CHUNK {chunk_idx}: {len(records) - len(new_records)} aspect(s) already existed in DB.")
    print(f"CHUNK {chunk_idx}: {len(new_records)} new aspect(s) will be upserted.")

    # === Reassign aspect IDs with offset based on existing aspects in DB ===
    for rec in new_records:
        review_id = rec['review_id']
        existing_count = len(existing_aspects_cache.get(review_id, []))
        existing_count += 1  # first new aspect will be existing_count + 0

        rec['aspect_number'] = existing_count
        rec['aspect_id'] = f"{review_id}_{rec['aspect_number']}"
        existing_aspects_cache[review_id].add(normalize_for_matching(rec['aspect']))

    print(f"CHUNK {chunk_idx}: Aspects to be inserted:")
    for rec in new_records:
        print(
            f"Review ID: {rec['review_id']}, Aspect: '{rec['aspect']}', Sentiment: {rec['sentiment']}, Snippet: {rec['snippet_4']}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if FILTER_MODE:
            print("[INFO] FILTER_MODE is ON → filtering specific review_ids...")
            df_filtered = filter_json_by_review_ids(filename, review_ids_to_keep)
            print(f"[INFO] Found {len(df_filtered)} matching reviews.")
            n_chunks = (len(df_filtered) + chunk_size - 1) // chunk_size
            chunk_iterator = enumerate(
                [df_filtered.iloc[i:i + chunk_size] for i in range(0, len(df_filtered), chunk_size)]
            )
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for chunk_idx, df_chunk in tqdm(chunk_iterator, total=total_chunks_to_process, desc="Submitting chunks", unit="chunk"):
                ...
                futures.append(future)
                print(f"Added chunk {chunk_idx} to futures")
